...
app.post('/charge', async (req, res) => {
  try {
    const { walletNumber, amount, payment_method } = req.body;
    ...
    // Fix applied: Removed automatic_payment_methods to avoid conflict with confirmation_method
    const charge = await stripe.paymentIntents.create({
      amount: Math.round(amount * 100),
      currency: 'usd',
      payment_method: payment_method,
      confirmation_method: 'manual',  // Keep confirmation method
      confirm: true,
    ...
    console.log(`Charged $${amount} from wallet ${walletNumber} using PaymentMethod ${payment_method}`);
